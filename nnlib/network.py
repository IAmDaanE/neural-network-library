...
class Network:
    ...
    def visualize(self):
        ...
        side_offset = 40
        hor_gap = (self.window_width - 2 * side_offset) / (self.hidden_amount + 1)
        node_gap = 22
        node_radius = 12
        for i in range(self.input_size):
            y = self.window_height / 2 - node_gap * (self.input_size / 2) + i * node_gap
            pygame.draw.circle(self.screen, (255,255,255), (side_offset, y), node_radius, 3)
        for q in range(self.hidden_amount):
            for i in range(self.hidden_size):
                y = self.window_height / 2 - node_gap * (self.hidden_size / 2) + i * node_gap
                pygame.draw.circle(self.screen, (255,255,255), (side_offset + hor_gap * (q + 1), y), node_radius, 3)
        for i in range(self.output_size):
            y = self.window_height / 2 - node_gap * (self.output_size / 2) + i * node_gap
            pygame.draw.circle(self.screen, (255,255,255), (side_offset + (self.hidden_amount + 1) * hor_gap, y), node_radius, 3)
            for q in range(self.hidden_amount):
                if q == 0:
                    start_x = side_offset
                    ...
                    for i in range(self.input_size):
                        ...
                        for p in range(self.hidden_size):
                            weight = self.layers[0].weights[i, p]
                            if weight > 0:
                                color = (255, 255, 255)
                            else:
                                color = (0, 134, 212)
                    ...
                    for i in range(self.hidden_size):
                        ...
                        for p in range(self.hidden_size):
                            weight = self.layers[q + 1].weights[i, p]
                            if weight > 0:
                                color = (255, 255, 255)
                            else:
                                color = (0, 134, 212)
            ...
            for q in range(self.hidden_size):
                start_x = side_offset + hor_gap * (self.hidden_amount)
                end_x = side_offset + hor_gap * (self.hidden_amount + 1)
